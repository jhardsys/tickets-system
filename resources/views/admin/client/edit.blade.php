...
@section('content')
    <div class="container mx-auto py-8">
        <form class="w-full max-w-2xl mx-auto bg-white p-8 rounded-md shadow-md " action="{{ route('admin.clients.update', ['client' => $client->id]) }}" method="POST">
            @csrf
            @method('PUT')
        <h1 class="text-2xl font-bold mb-6 text-center text-gray-700">Actualizar cliente</h1>
            <div class="grid grid-cols-2 gap-6">
                <div class="mb-4">
                    ...
                    <input class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500" type="text" id="first_name" name="first_name" value="{{ $client->first_name }}">
                </div>
            
                <div class="mb-4">
                    <label class="block text-gray-700 text-xl font-bold mb-2" for="first_surname">Primer Apellido:</label>
                    <input class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500" type="text" id="first_surname" name="first_surname" value="{{ $client->first_surname }}">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div class="mb-4">
                    ...
                    <input class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500" type="text" id="second_surname" name="second_surname" value="{{ $client->second_surname }}">
                </div>
                
                <div class="mb-4">
                    <label class="block text-gray-700 text-xl font-bold mb-2" for="second_surname">Phone</label>
                    <input class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500"  id="phone" name="phone" value="{{ $client->phone }}">
                </div>
            </div>
